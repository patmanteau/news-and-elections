...
def mentions_per_section_per_week(input_filepath, output_filepath):
    ...
    mentions = df.groupby([pd.Grouper(key='tstamp', freq='W'), 'section', 'wd_id']).size().reset_index(name='count')
...
mentions_per_section_per_week(snakemake.input[0], snakemake.output[0])